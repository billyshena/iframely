...
var prepareRequestOptions = (exports.prepareRequestOptions = function(
  request_options
) {
  if (CONFIG.PROXY) {
    ...
    try {
      ...
      var r = request(
        ...
        .on('response', function(res) {
          if (
            supportGzip &&
            ['gzip', 'deflate'].indexOf(res.headers['content-encoding']) > -1
          ) {
  ...
  cache.withCache(
    ...
    function(callback) {
      ...
      function finish(error, data) {
        ...
        if (
          (typeof error === 'string' && error.indexOf('ENOTFOUND') > -1) ||
          error === 500
        ) {
          error = 404
        }
              ...
              .on('response', function(res) {
                ...
                if (
                  ...
                  if (!uri.match(/\.(jpg|png|gif)(\?.*)?$/i)) {
                    cb(
                      'invalid content type: no content-type header and file extension'
                    )
                  }
                }
...
exports.generateLinksHtml = function(data, options) {
  ...
  iterateLinks(links, function(link) {
    if (!link.html && !link.type.match(/^image/)) {
      ...
      if (
        options.autoplayMode &&
        link.type.indexOf('video/') === 0 &&
        link.rel.indexOf('autoplay') === -1
      ) {
...
function isYoutube(meta) {
  ...
  if (meta.og && (video = meta.og.video)) {
    ...
    for (var i = 0; i < video.length; i++) {
      ...
      if (
        v.secure_url &&
        v.secure_url.indexOf &&
        v.secure_url.indexOf('youtube') > -1
      ) {
        return true
      }
    }
...
function getWhitelistLogData(meta, oembed) {
  ...
  if (meta) {
    ...
    var isShopify = false
    if (meta.alternate) {
      var alternate = meta.alternate instanceof Array
        ? meta.alternate
        : [meta.alternate]
      var oembedLink
    ...
    r.twitter_photo =
      ...
      ((meta.twitter && !!meta.twitter.image) || (meta.og && !!meta.og.image))

    r.twitter_player = meta.twitter && !!meta.twitter.player

    r.twitter_stream =
      meta.twitter && meta.twitter.player && !!meta.twitter.player.stream

    r.og_video = meta.og && !!meta.og.video && !isYoutube(meta)

    r.video_src = !!meta.video_src

    r.sm4_video = !!(meta.sm4 && meta.sm4.video && meta.sm4.video.embed)
  }

  if (
    oembed &&
    oembed.type !== 'link' &&
    !(oembed.type == 'rich' && isWordpress)
  ) {
